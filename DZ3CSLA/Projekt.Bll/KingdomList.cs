...
{
    // Read-only popis kraljevstva (cache).
    [Serializable()]
    public class KingdomList : NameValueListBase<int, string>
    {
        #region Constructors
        private KingdomList()
        {
        ...
        private static KingdomList list;

        public static KingdomList Get()
        {
            if (list == null)
            {
                list = DataPortal.Fetch<KingdomList>();
            }
            return list;
        }
        ...
        private void DataPortal_Fetch()
        {
            ...
            using (var ctx = Dal.ContextManager<Dal.BTVDataContext>.GetManager(Dal.Database.BTVConnectionString))
            {
                ...
                foreach (var u in ctx.DataContext.Kingdom_t.AsNoTracking().ToList())
                {
