...
{
    // Read-only popis divizija (cache).
    [Serializable()]
    public class PhylumList : NameValueListBase<int, string>
    {
        #region Constructors
        private PhylumList()
        {
        ...
        private static PhylumList list;

        public static PhylumList Get()
        {
            if (list == null)
            {
                list = DataPortal.Fetch<PhylumList>();
            }
            return list;
        }
        ...
        private void DataPortal_Fetch()
        {
            ...
            using (var ctx = Dal.ContextManager<Dal.BTVDataContext>.GetManager(Dal.Database.BTVConnectionString))
            {
                ...
                foreach (var u in ctx.DataContext.Phylum_t.AsNoTracking().ToList())
                {
