...
{
    // Read-only popis vrsti (cache).
    [Serializable()]
    public class SpeciesList : NameValueListBase<int, string>
    {
        #region Constructors
        private SpeciesList()
        {
        ...
        private static SpeciesList list;

        public static SpeciesList Get()
        {
            if (list == null)
            {
                list = DataPortal.Fetch<SpeciesList>();
            }
            return list;
        }
        ...
        private void DataPortal_Fetch()
        {
            ...
            using (var ctx = Dal.ContextManager<Dal.BTVDataContext>.GetManager(Dal.Database.BTVConnectionString))
            {
                ...
                foreach (var u in ctx.DataContext.Species_t.AsNoTracking().ToList())
                {
