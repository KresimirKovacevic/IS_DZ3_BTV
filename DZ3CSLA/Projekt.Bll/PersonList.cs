...
{
	// Read-only popis osoba (cache).
	[Serializable()]
	public class PersonList : NameValueListBase<int, string>
	{
		#region Constructors
		private PersonList()
		{
		...
		private static PersonList list;

		public static PersonList Get()
		{
			if (list == null)
			{
				list = DataPortal.Fetch<PersonList>();
			}
			return list;
		}
		...
		private void DataPortal_Fetch()
		{
			...
			using (var ctx = Dal.ContextManager<Dal.BTVDataContext>.GetManager(Dal.Database.BTVConnectionString))
			{
				...
				foreach (var u in ctx.DataContext.People.AsNoTracking().ToList())
				{
					data.Add(new NameValuePair(u.ID, u.Surname+", "+u.Name));
				}
